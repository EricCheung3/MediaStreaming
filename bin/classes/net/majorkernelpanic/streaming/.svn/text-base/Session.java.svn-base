 ...
 * 
 * If you don't use the RTSP protocol, you will still need to send a session
 * description to the receiver for him to be able to decode your audio/video
...
public class Session {
	...
	/**
	 * The phone may not support some streaming parameters that you are trying
	 * to use (bit rate, frame rate, resolution...).
	 */
	public final static int ERROR_CONFIGURATION_NOT_SUPPORTED = 0x01;

	/**
	 * The internal storage of the phone is not ready. libstreaming tried to
	 * store a test file on the sdcard but couldn't. See H264Stream and
	 * AACStream to find out why libstreaming would want to something like that.
	 */
	...
	/**
	 * The supplied SurfaceView is not a valid surface, or has not been created
	 * yet.
	 */
	public final static int ERROR_INVALID_SURFACE = 0x04;

	/**
	 * The destination set with {@link Session#setDestination(String)} could not
	 * be resolved. May mean that the phone has no access to the internet, or
	 * that the DNS server could not resolved the host name.
	 */
	...
	/**
	 * The callback interface you need to implement to get some feedback Those
	 * will be called from the UI thread.
	 */
	public interface Callback {

		/**
		 * Called periodically to inform you on the bandwidth consumption of the
		...
		/**
		 * Called when the previw of the {@link VideoStream} has correctly been
		 * started. If an error occurs while starting the preview,
		 * {@link Callback#onSessionError(int, int, Exception)} will be called
		 * instead of {@link Callback#onPreviewStarted()}.
		 */
		public void onPreviewStarted();

		/**
		 * Called when the session has correctly been configured after calling
		 * {@link Session#configure()}. If an error occurs while configuring the
		 * {@link Session}, {@link Callback#onSessionError(int, int, Exception)}
		 * will be called instead of {@link Callback#onSessionConfigured()}.
		 */
		 ...
		 * {@link Callback#onSessionError(int, int, Exception)} will be called
		 * instead of {@link Callback#onSessionStarted()}.
		 */
	...
	/**
	 * You probably don't need to use that directly, use the
	 * {@link SessionBuilder}.
	 */
	...
	/**
	 * The origin address of the session. It appears in the session description.
	 * 
	 * @param origin
	 *            The origin address
	 */
	...
	/**
	 * Returns a Session Description that can be stored in a file or sent to a
	 * client with RTSP.
	 * 
	 * @return The Session Description.
	 * @throws IllegalStateException
	 *             Thrown when {@link #setDestination(String)} has never been
	 *             called.
	 */
...
//		sessionDescription.append("c=IN IP4 " + "124.193.154.2" + "\r\n");
		...
		if (mVideoStream != null) {
			String vsdp = mVideoStream.getSessionDescription();
//			if (mAudioStream != null) {
//				vsdp = vsdp.replace(" RTP/AVP 96\r\n", " RTP/AVP 97\r\n");
//				vsdp = vsdp.replace("a=rtpmap:96","a=rtpmap:97");
//				vsdp = vsdp.replace("a=fmtp:96","a=fmtp:97");
//			}
			sessionDescription.append(vsdp);
	...
	/**
	 * Returns an approximation of the bandwidth consumed by the session in bit
	 * per second.
	 */
	...
	public boolean isStreaming() {
		if ((mAudioStream != null && mAudioStream.isStreaming())
	 ...
	 * occurs.
	 **/
	public void syncConfigure() throws CameraInUseException,
			StorageUnavailableException, ConfNotSupportedException,
			...
			public void run() {
				if (mVideoStream != null) {
					try {
						...
						postError(ERROR_CONFIGURATION_NOT_SUPPORTED,
								STREAM_VIDEO, e);
					} catch (InvalidSurfaceException e) {
	 ...
	 * of the flash with {@link Session#getVideoTrack()} and
	...
	private void postError(final int reason, final int streamType,
			final Exception e) {
		mMainHandler.post(new Runnable() {
			@Override
			public void run() {
