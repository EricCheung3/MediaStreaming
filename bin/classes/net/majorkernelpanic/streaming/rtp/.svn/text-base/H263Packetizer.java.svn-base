 ...
 * 
 * Must be fed with an InputStream containing H.263 frames. The stream must
 * start with mpeg4 or 3gpp header, it will be skipped.
 * 
 */
	...
	public void run() {
		...
		try {
			...
			while (!Thread.interrupted()) {
				...
				// Each packet we send has a two byte long header (See section
				// 5.1 of RFC 4629)
				buffer[rtphl] = 0;
				buffer[rtphl + 1] = 0;
				...
				// Log.d(TAG,"j: "+j+" buffer: "+printBuffer(rtphl,
				// rtphl+5)+" tr: "+tr);
				if (firstFragment) {
					// This is the first fragment of the frame -> header is set
					// to 0x0400
					buffer[rtphl] = 4;
					firstFragment = false;
				} else {
