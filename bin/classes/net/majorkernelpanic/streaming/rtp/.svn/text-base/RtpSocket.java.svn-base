...
public class RtpSocket implements Runnable {
	...
	/**
	 * data is same as buffer,length is set when data commit,send this is equal
	 * send buffer
	 */
	...
	/**
	 * tcp has a header,while udp don't
	 */
	...
	public void start() {
		if (mThread == null) {
			mThread = new Thread(this, TAG + " - " + Thread.currentThread().getName());
			mThread.start();
		}
	}
	...
	public void setConsumer(RtpThread thread, byte channelIdentifier) {
		if (thread != null) {
			...
			mConsumer = thread;
			mTcpHeader[1] = channelIdentifier;
			mReport.setConsumer(thread, (byte) (channelIdentifier + 1));
		}
	}
	 ...
	 **/
	public void updateTimestamp(long timestamp) {// 90000
		mTimestamps[mBufferIn] = timestamp;
		long n = (timestamp / 100L) * (mClock / 1000L) / 10000L;
		// ts /100/1000 * 10; = ts/10000
		// Log.i(Thread.currentThread().getName(), timestamp + ",  " +
		// String.valueOf(n));
		setLong(mBuffers[mBufferIn], n, 4, 8);
	}
	...
	public void run() {
		...
		try {
			...
			long delta = 0;
			while (mThread != null && mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
				if (mOldTimestamp != 0) {
				...
				mReport.update(mPackets[mBufferOut].getLength(), (mTimestamps[mBufferOut] / 100L)
						* (mClock / 1000L) / 10000L);
	...
	private void sendTCP() throws IOException, InterruptedException {
		int len = mPackets[mBufferOut].getLength();
		mTcpHeader[2] = (byte) (len >> 8);
		mTcpHeader[3] = (byte) (len & 0xFF);
		RtpThread consumer = mConsumer;
		if (consumer != null) {
			ByteBuffer buffer = consumer.available(mTcpHeader.length + len);
			buffer.clear();
			buffer.put(mTcpHeader);
			buffer.put(mBuffers[mBufferOut], 0, len);
			buffer.flip();
			consumer.commitWrite(buffer);
			return;
		}
		synchronized (mOutputStream) {
			...
			// ByteBuffer buffer = ByteBuffer.wrap(mBuffers[mBufferOut]);
			// int seq = buffer.getShort(2);
			// int stamp = buffer.getInt(4);
			// Log.d(TAG, "seq: " + seq + ", stamp: " + stamp + ", len: " +
			// len);
