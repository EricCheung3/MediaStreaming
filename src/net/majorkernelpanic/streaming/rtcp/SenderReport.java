...
public class SenderReport {
	...
	private RtpThread mConsumer = null;
	...
	public SenderReport() {
		...
		try {
			...
			// Very unlikely to happen. Means that all UDP ports are already
			// being used
	...
	/**
	 * Sets the temporal interval between two RTCP Sender Reports. Default
	 * interval is set to 3 secondes. Set 0 to disable RTCP.
	 * 
	 * @param interval
	 ...
	 **/
	public void update(int length, long rtpts) throws IOException, InterruptedException {
		mPacketCount += 1;
		mOctetCount += length;
	...
	/**
	 * Resets the reports (total number of bytes sent, number of packets sent,
	 * etc.)
	 */
	public void reset() {
		mPacketCount = 0;
	 ...
	 * @throws InterruptedException */
	private void send(long ntpts, long rtpts) throws IOException, InterruptedException {
		...
		if (mTransport == TRANSPORT_UDP) {
			...
			if (consumer != null) {
				ByteBuffer buffer = consumer.available(mTcpHeader.length + PACKET_LENGTH);
				buffer.clear();
				buffer.put(mTcpHeader);
				buffer.put(mBuffer, 0, PACKET_LENGTH);
				buffer.flip();
